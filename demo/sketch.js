let sketch = function(p) {
  ...
  p.preload = function() {                                                      // preload images and sounds
    maskImg = p.loadImage('images/mask.png');
    tileImg = p.loadImage('images/tile.png');
    mineImg = p.loadImage('images/mine.png');
    flagImg = p.loadImage('images/flag.png');
    explImg = p.loadImage('images/expl.png');
    diffImg = p.loadImage('images/diff.png');
    wrngImg = p.loadImage('images/wrng.png');
    num0Img = p.loadImage('images/num0.png');
    num1Img = p.loadImage('images/num1.png');
    num2Img = p.loadImage('images/num2.png');
    num3Img = p.loadImage('images/num3.png');
    num4Img = p.loadImage('images/num4.png');
    num5Img = p.loadImage('images/num5.png');
    num6Img = p.loadImage('images/num6.png');
    num7Img = p.loadImage('images/num7.png');
    num8Img = p.loadImage('images/num8.png');
    p.soundFormats('wav');
    winSfx = p.loadSound('sfx/win');
    openSfx = p.loadSound('sfx/open');
    flagSfx = p.loadSound('sfx/flag');
    explodeSfx = p.loadSound('sfx/explode');
    numImgs = [num0Img, num1Img, num2Img, num3Img, num4Img, num5Img, num6Img, num7Img, num8Img];
  }

  p.setup = function() {                                                        // ran once
  ...
  reset = function() {
    ...
    size = Math.min(p.windowWidth, p.windowHeight) / ((xsize + ysize) * margin / 2); // size of one tile
    ...
    p.draw();                                                                   // draw the game
  }

  p.windowResized = function() {                                                // responsive design :)
    p.resizeCanvas(p.windowWidth, p.windowHeight);
    size = Math.min(p.windowWidth, p.windowHeight) / (xsize + ysize) * 2;
  ...
  p.touchEnded = function() {
    ...
    if (p.mouseX < 0 || p.mouseX > p.width || p.mouseY < 0 || p.mouseY > p.height)  { // if clicked outside canvas
  ...
  p.draw = function() {
    p.background(245);
    p.push();
    p.translate((p.width / 2 - size / 2) - _x, (p.height / 2 - size * 0.75 * 0.1) - _y); // set origin point on canvas
  ...
  function hitbox() {                                                           // stores hitboxes of tiles in a graphic object which is an off-screen canvas
    pg = p.createGraphics(p.width, p.height);                                   // create an off-screen canvas
    pg.background(0, 0, 255);                                                   // make it blue
    pg.translate((p.width / 2 - size / 2) - _x, (p.height / 2 - size * 0.75 * 0.1) - _y); // set the origin point so everything stays centered
  ...
  function isodraw(img, i) {
    let offx = i % xsize * size / 3 * margin;                                   // cartesian x offset
    let offy = Math.floor(i / xsize) * size / 3 * margin;                       // cartesian y offset`
    p.image(img, root3 * 0.82 * (offx - offy), 1 * (offx + offy), size, size * 0.75);// empty tile
  }
  ...
  function getTile() {                                                          // GENIUS part
    let c = pg.get(p.mouseX, p.mouseY);                                         // pick color from off screen canvas
    if (c[2] === 255) return false;                                             // if background color picked return false
    let i = c[0] + c[1] * xsize;                                                // calculate index of tile from color channels
